...
var $data = {};
var $lib = { Classic: {}, Jaqwi: {}, Crossword: {}, Typing: {}, Hunmin: {}, Daneo: {}, Sock: {}, Sansung: {} };
var $rec;
var mobile;
...
$("#intro").show();
	...
	$stage.chatBtn.on('click', function(e){
		...
		if(o.value[0] == "/"){
			...
			// 산성비 게임 중일 때 단어 제출
			if($data.room && $data.room.gaming && MODE[$data.room.mode] === 'KSB'){
				send('sansung-input', { value: o.value });
				$stage.talk.val("");
				$stage.game.hereText.val("");
				return;
			}
...
function onMessage(data){
	...
	switch (data.type) {
		...
		case 'error':
			...
			break;
			case 'sansung-spawn':
				$lib.Sansung.spawn(data);
				break;
			case 'sansung-hit':
				$lib.Sansung.hit(data);
				break;
			case 'sansung-miss':
				$lib.Sansung.miss(data);
				break;
			case 'sansung-end':
				$lib.Sansung.end(data);
				break;
