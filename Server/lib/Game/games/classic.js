...
const ROBOT_HIT_LIMIT = [ 10, 15, 25, 50, 100 ];
		...
		function preApproved(){
			function approved(){
				...
				if(!client.robot){
					...
					DB.kkutu[tk].update([ '_id', text ]).set([ 'hit', $doc.hit + 1 ]).on();
				}
			}
...
exports.readyRobot = function(robot){
	...
	getAuto.call(my, my.game.char, my.game.subChar, 2).then(function(list){
		...
		function getWish(char){
			...
			DB.kkutu[tk].find([ '_id', new RegExp(isRev ? `.${char}$` : `^${char}.`) ]).limit(10).on(function($res){
...
function getAuto(char, subc, type){
	...
	function produce(){
		...
		var tk = getTableKey.call(my);
		DB.kkutu[tk].find.apply(this, aqs).limit(bool ? 1 : 123).on(function($md){
			forManner($md);
...
function getTableKey() {
	...
	if (lang === "ko" && my.opts && my.opts.chundo) {
		return "ko_chundo";
	}
	return lang;
}
