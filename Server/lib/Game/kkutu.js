...
/* 망할 셧다운제
 e x*ports.processAjae = function(){
 var i;
...
exports.Client = function(socket, profile, sid){
	...
	*/
	...
	/* 망할 셧다운제
	 m *y.confirmAjae = function(input){
...
exports.onClientClosed = function(client, code){
	// 기본 처리: 별도 game.js 등에서 오버라이드될 수 있음
};
exports.onClientMessage = function(client, data){
	var room = ROOM[client.place];

	// 산성비 단어 입력 처리
	if(data.type === 'sansung-input'){
		if(room && room.game && room.game.sansung){
